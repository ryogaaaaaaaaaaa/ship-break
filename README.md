...
- 成功画面で `FIX` / `WORKAROUND` / `EXPLOIT` を選び、次ランの `TIME DESYNC` 挙動を変えられる。
- `WORKAROUND` は全体スロー、`EXPLOIT` はプレイヤーだけ通常速度、`FIX` は通常時間へ戻す。
...
60秒生存するとデバッグ割り込み画面が出る。`FIX` / `WORKAROUND` / `EXPLOIT` のいずれかを選ぶと、次のランへ進む。
