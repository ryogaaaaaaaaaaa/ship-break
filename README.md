...
- 成功画面から再起動すると、`TIME DESYNC` の `WORKAROUND` が適用される。
...
60秒生存するとデバッグ割り込み画面が出る。タップまたはキー入力で `TIME DESYNC WORKAROUND` を適用し、次のランへ進む。
現時点で接続済みなのは `WORKAROUND` 1本だけで、`FIX` と `EXPLOIT` はまだ未接続。
